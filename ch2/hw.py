...
print(np.cov(np.array(x[:sample_num/2]).T))

#neigh = KNeighborsClassifier(n_neighbors=3, p=2, metric='minkowski')
neigh = KNeighborsClassifier(n_neighbors=3, metric='mahalanobis', V=np.cov(np.array(x[:sample_num/2]).T))
neigh.fit(x[:sample_num/2],y[:sample_num/2])

print(x[sample_num/2:])
...
#predict values
for j in range(sample_num/2, 57) :
...
#original values
for k in range(sample_num/2, 57) : 
	if y[k] == 0 :
